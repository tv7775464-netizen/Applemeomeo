...
local ConfigInput = Tabs.Config:AddInput("ConfigName", {
    Title = "Config Name",
    Default = "Default",
...
local function getConfigPath()
    return ConfigFolder .. "/" .. (ConfigName ~= "" and ConfigName or "Default") .. ".txt"
end

Tabs.Config:AddButton({
    Title = "Save Config",
    Description = "Lưu trạng thái config",
    Callback = function()
        if writefile then
            pcall(function()
                writefile(getConfigPath(), "{}")
            end)
        end
    end
})
...
Tabs.Config:AddButton({
    Title = "Load Config",
    Description = "Tải config đã lưu",
    Callback = function()
        if isfile and isfile(getConfigPath()) then
            -- Config file tồn tại; các option cụ thể có thể được nối vào đây.
        end
    end
})
...
Tabs.Config:AddButton({
    Title = "Delete Config",
    Description = "Xoá config hiện tại",
    Callback = function()
        if isfile and isfile(getConfigPath()) and delfile then
            pcall(function()
