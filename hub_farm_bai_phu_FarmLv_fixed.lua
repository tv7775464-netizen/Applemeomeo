...
-- Secondary farm fallback for Auto Farm Level.
-- When the main quest mob is not spawned, farm another nearby live mob
-- temporarily, then return to the main mob as soon as it appears.
local function FarmSecondaryWhileWaiting(mainEnemyName)
    if not _G.Level then return false end

    local enemies = workspace:FindFirstChild("Enemies")
    if enemies then
        for _, mob in ipairs(enemies:GetChildren()) do
            if mob.Name ~= mainEnemyName and Attack.Alive(mob) then
                local hrp = mob:FindFirstChild("HumanoidRootPart")
                if hrp then
                    _tp(hrp.CFrame * CFrame.new(0, 20, 0))
                    repeat
                        task.wait(Sec)
                        if not _G.Level or not mob.Parent or not Attack.Alive(mob) then
                            break
                        end
                        _tp(hrp.CFrame * CFrame.new(0, 20, 0))
    ...
    -- No live secondary mob: move to another spawn instead of idling.
    local worldOrigin = workspace:FindFirstChild("_WorldOrigin")
    ...
    if spawnFolder then
        for _, spawnPoint in ipairs(spawnFolder:GetChildren()) do
            if spawnPoint:IsA("BasePart")
                and not string.find(spawnPoint.Name, mainEnemyName, 1, true) then
            ...
            pcall(function()
                ...
                elseif plr.PlayerGui.Main.Quest.Visible == true then
                    if workspace.Enemies:FindFirstChild(QuestNeta()[1]) then
                        ...
                        -- Main mob is not spawned: farm a secondary mob instead of idling.
                        local usedSecondary = FarmSecondaryWhileWaiting(QuestNeta()[1])
                        if not usedSecondary then
                            -- No secondary mob found: move to the quest/mob area once,
                            -- then let the next loop retry instead of spamming teleports.
                            local questPos = QuestNeta()[4]
                            if questPos then
                                local dist = (Root.Position - questPos.Position).Magnitude
                                if dist > 80 then
                                    _tp(questPos)
                                end
                            end
...
Tabs.Raids:AddToggle({
    Name = "Kill Aura Raid",
    Description = "Kill enemies during raids around the current island",
    Default = false,
    Callback = function(Value)
