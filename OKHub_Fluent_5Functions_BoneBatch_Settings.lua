...
Tabs.Main:AddDropdown({
    Name = "Gom Bone",
    Description = "So luong quai moi bai",
    Options = {"2", "4", "6", "8"},
    Default = "6",
    Callback = function(Value)
        _G.BoneBatchCount = tonumber(Value) or 6
        _G.BoneBatch = {}
        _G.BoneBatchIndex = 0
    end
})
...
Tabs.Main:AddToggle({
    Name = "Auto Farm Bone",
    Description = "Gom du so quai roi moi chuyen bai",
    Default = false,
    Callback = function(Value)
...
spawn(function()
    ...
    local function isBoneMob(v)
        if not v or not v.Parent then return false end
        if not table.find(BonesTable, v.Name) then return false end
    ...
    local function makeBatch()
        ...
        _G.BoneBatch = {}
        _G.BoneBatchIndex = 0
        for i = 1, math.min(_G.BoneBatchCount, #mobs) do
            _G.BoneBatch[i] = mobs[i]
        end
        return #_G.BoneBatch
    end
    ...
    while wait(0.15) do
        ...
        pcall(function()
            ...
            if _G.AcceptQuestB and questUI and not questUI.Visible then
                _tp(CFrame.new(-9516.99316,172.01718,6078.46533))
                repeat task.wait(0.2)
                until not _G.AutoFarm_Bone
                    or (Vector3.new(-9516.99316,172.01718,6078.46533) - root.Position).Magnitude <= 50
            ...
            -- Tao mot batch co dinh. Khong doi batch cho den khi tat ca quai trong batch chet.
            if #_G.BoneBatch == 0 then
                local amount = makeBatch()
                if amount == 0 then
                    _tp(boneFarmPos)
                    return
                end
            ...
            -- Loai bo mob da chet/bi xoa khoi batch.
            while _G.BoneBatchIndex < #_G.BoneBatch do
                _G.BoneBatchIndex = _G.BoneBatchIndex + 1
                local mob = _G.BoneBatch[_G.BoneBatchIndex]

                if isBoneMob(mob) then
                    repeat
                        task.wait()
                        if not _G.AutoFarm_Bone then return end
            ...
            -- Chi khi ca batch da xu ly xong moi xoa batch va chuyen sang bai moi.
            if _G.BoneBatchIndex >= #_G.BoneBatch then
                _G.BoneBatch = {}
                _G.BoneBatchIndex = 0
                task.wait(0.3)
            end
