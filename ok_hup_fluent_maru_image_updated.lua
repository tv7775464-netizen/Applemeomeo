...
local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/xshiba/OBFUP/refs/heads/main/GET.lua "))()
...
-- Compatibility layer: keeps the original hub features while using Fluent Maru.
local _FluentId = 0
local function _FluentMakeId(name)
    _FluentId = _FluentId + 1
    name = tostring(name or "Item"):gsub("[^%w_]", "_")
    return "Item_" .. name .. "_" .. _FluentId
end
...
local function _WrapTab(tab)
    ...
    function T:AddParagraph(title, desc)
        local obj = tab:AddParagraph({
    ...
    function T:AddToggle(cfg)
        cfg = cfg or {}
        return tab:AddToggle(_FluentMakeId(cfg.Name or cfg.Title), {
    ...
    function T:AddDropdown(cfg)
        cfg = cfg or {}
        local values = cfg.Options or cfg.Values or {}
        return tab:AddDropdown(_FluentMakeId(cfg.Name or cfg.Title), {
    ...
    function T:AddSlider(cfg)
        cfg = cfg or {}
        local increment = cfg.Increment or 1
        local rounding = 0
        if increment < 1 then
            rounding = math.max(0, math.ceil(-math.log10(increment)))
        end
        return tab:AddSlider(_FluentMakeId(cfg.Name or cfg.Title), {
    ...
    function T:AddTextBox(cfg)
        cfg = cfg or {}
        return tab:AddInput(_FluentMakeId(cfg.Name or cfg.Title), {
            Title = cfg.Name or cfg.Title or "Input",
            Description = cfg.Description or "",
            Default = cfg.Default or "",
            Placeholder = cfg.Placeholder or "",
            ClearOnFocus = cfg.ClearOnFocus == true,
            Finished = true,
            Callback = cfg.Callback or function() end
        })
    end
...
function Window:SetUIScale(scale)
    -- Fluent handles window sizing internally; keep this method for compatibility.
end

function Window:Notify(cfg)
    return Fluent:Notify(cfg)
end

local Tabs = {
    Info = _WrapTab(Window:AddTab({ Title = "Tab Info And Status", Icon = "info" })),
    Main = _WrapTab(Window:AddTab({ Title = "Tab Farming", Icon = "swords" })),
    Settings = _WrapTab(Window:AddTab({ Title = "Tab Setting", Icon = "settings" })),
    Fish = _WrapTab(Window:AddTab({ Title = "Tab Fishing", Icon = "fish" })),
    Quests = _WrapTab(Window:AddTab({ Title = "Tab Quest And Item", Icon = "scroll" })),
    SeaEvent = _WrapTab(Window:AddTab({ Title = "Tab Sea Event", Icon = "waves" })),
    Race = _WrapTab(Window:AddTab({ Title = "Tab Mirage And Race", Icon = "zap" })),
    Prehistoric = _WrapTab(Window:AddTab({ Title = "Tab Volcano Event", Icon = "tent" })),
    Esp = _WrapTab(Window:AddTab({ Title = "Tab Stats And Esp", Icon = "eye" })),
    Raids = _WrapTab(Window:AddTab({ Title = "Tab Fruit And Raid", Icon = "gem" })),
    Combat = _WrapTab(Window:AddTab({ Title = "Tab Local Player", Icon = "user" })),
    Travel = _WrapTab(Window:AddTab({ Title = "Tab Teleport", Icon = "map-pin" })),
    Shop = _WrapTab(Window:AddTab({ Title = "Tab Shopping", Icon = "shopping-bag" })),
    Misc = _WrapTab(Window:AddTab({ Title = "Tab Miscellaneous", Icon = "more-horizontal" }))
}
...
Description = "",
Callback = function()
  for i, v in pairs(game:GetObjects("rbxassetid://122325089010707")[1]:GetChildren()) do
...
Window:Notify({
  Title = "Apple meomeo hub",
  Content = "free bro",
  Image = "rbxassetid://122325089010707",
  Duration = 5
})