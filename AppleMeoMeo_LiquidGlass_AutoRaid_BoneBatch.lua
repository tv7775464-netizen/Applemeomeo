                ...
                if v ~= Mon and v.Parent == workspace.Enemies then
                    ...
                    if hum and root and hum.Health > 0 then
                        ...
                        if distance <= 3000 then
                            ...
                            if not areaMob then
                                local networkOwned = isnetworkowner and isnetworkowner(root)
...
local BoneGroupCount = 6
Tabs.Main:AddDropdown({
    Name = "Gom Quái",
    Description = "Số quái gom trước khi chuyển bãi",
    Options = {"2", "4", "6", "8"},
    CurrentOption = {"6"},
    ...
    Callback = function(Value)
        ...
        BoneGroupCount = tonumber(v) or 6
    end
})

Tabs.Main:AddToggle({
    Name = "Auto Farm Bone",
    Description = "Gom đủ số quái đã chọn, giết hết rồi mới đổi bãi",
    Default = false,
    Callback = function(Value)
        _G.AutoFarm_Bone = Value
        if not Value then
            _G.BoneFarmBatch = nil
            _G.BoneFarmAreaIndex = 1
        end
    end
})
...
spawn(function()
    ...
    -- Các bãi Bone trong Haunted Castle. Sau mỗi batch chết hết sẽ đổi bãi.
    local BoneAreas = {
        {"Reborn Skeleton", CFrame.new(-8764, 142, 5963)},
        {"Living Zombie", CFrame.new(-10227, 421, 6161)},
        {"Demonic Soul", CFrame.new(-9579, 6, 6194)},
        {"Possessed Mummy", CFrame.new(-9579, 6, 6194)}
    }

    local function GetAliveBones()
        local result = {}
        for _, mobName in ipairs(BonesTable) do
            local bucket = EnemyCache[mobName]
            if bucket then
                for mob in pairs(bucket) do
                    if mob.Parent == workspace.Enemies and Attack.Alive(mob)
    ...
    local function BuildBatch()
        local alive = GetAliveBones()
        local batch = {}
        local limit = math.min(BoneGroupCount, #alive)
        for i = 1, limit do
            batch[i] = alive[i]
        end
        return batch
    ...
    while wait(0.25) do
        ...
        pcall(function()
            ...
            -- Nếu chưa có batch thì gom một batch mới.
            if not _G.BoneFarmBatch or #_G.BoneFarmBatch == 0 then
                local area = BoneAreas[_G.BoneFarmAreaIndex or 1]
                if area and (root.Position - area[2].Position).Magnitude > 180 then
                    _tp(area[2])
                    return
                end

                local batch = BuildBatch()
                if #batch >= BoneGroupCount then
                    _G.BoneFarmBatch = batch
                elseif #batch > 0 then
                    -- Chờ đủ số quái, không đổi bãi giữa chừng.
                    for _, mob in ipairs(batch) do
                        if mob.Parent and Attack.Alive(mob) then
            ...
            -- Chỉ đánh những con thuộc batch hiện tại.
            local aliveInBatch = 0
            for i = #_G.BoneFarmBatch, 1, -1 do
                local mob = _G.BoneFarmBatch[i]
                if mob and mob.Parent == workspace.Enemies and Attack.Alive(mob) then
                    aliveInBatch += 1
                    Attack.Kill(mob, true)
                else
                    table.remove(_G.BoneFarmBatch, i)
                end
            end

            -- Chỉ khi toàn bộ batch đã chết mới chuyển sang bãi tiếp theo.
            if aliveInBatch == 0 or #_G.BoneFarmBatch == 0 then
                _G.BoneFarmBatch = nil
                _G.BoneFarmAreaIndex = (_G.BoneFarmAreaIndex or 1) % #BoneAreas + 1
                wait(0.15)
            end
        end)
    end
